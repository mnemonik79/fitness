...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	rez := (CaloriesMeanSpeedMultiplier*r.Training.meanSpeed() + CaloriesMeanSpeedShift) * float64(r.Training.Weight) / float64(MInKm) * float64(MinInHours)
	return rez
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	rezult := CaloriesWeightMultiplier * w.Training.Weight
	if w.Height != 0 {
		rezult = (rezult + (math.Pow(w.Training.meanSpeed()*KmHInMsec, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Training.Weight)
	}
	rezult = rezult * MinInHours * w.Training.Duration.Hours()
	return rezult
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	return (s.Training.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Training.Weight * s.Training.Duration.Hours()
}
